...
const unlovable_ids = unlovables.map((item, index) => { return breedIds[item]; });
const url = `${apiUrl}types/dog/breeds?exclude_breeds=${encodeURIComponent(unlovables)}`;
console.log(url)
fetch(url, {})
...
.then(data => {
  ...
  data.breeds.forEach(breed => {
    const url = `${apiUrl}animals?type=dog&breed=${breed.name}&location=San Francisco, CA&limit=100`;
    ...
    .then(data => {
      if (data.animals.length > 0) {
        ...
        animalsList.appendChild(breedHeading);
        ...
        data.animals.forEach(animal => {
            ...
            animalImage.src = animal.photos.length > 0 ? animal.photos[0].medium : "placeholder.jpg";
        ...
        animalsList.appendChild(breedList);
    }
    })
